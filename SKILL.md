...
- Preserve every result required by a protocol, venue, or reproducibility standard, together with unfavorable evidence that materially changes the headline claim, safety, fairness, leakage assessment, or applicability. A manuscript need not enumerate every exploratory null, abandoned variant, tuning run, or secondary analysis that has no bearing on the final claim; omit or compress such material by default. If that search determined a highlighted result, disclose the relevant search scope, data reuse, and multiplicity in aggregate rather than as a failure diary.
- Outcome-informed choices of endpoints, metrics, baselines, subgroups, thresholds, datasets, or comparison framing MAY support a post hoc exploratory analysis or the manuscript's exploratory narrative when they have a substantive rationale and their selection context is disclosed where it affects interpretation. Do not silently present them as prespecified or confirmatory, let them replace a required primary analysis or declared final evaluation, or use them to conceal a material contradiction. Retain the internal analysis record when policy, provenance, or later verification requires it.
...
- Identify the single strongest evidence-supported contribution and make it the organizing center. Lead with the strongest result supported under its actual evidential status and the minimum credible evidence chain; nominal favorability after outcome-informed selection is not by itself robustness. Supporting analyses should explain, validate, delimit, or operationalize that contribution instead of appearing as an equal-weight experiment log; non-consequential failed pilots, exploratory nulls, and discarded variants may be grouped, moved to a supplement, or omitted.
...
- Include an unfavorable or null result in the publishable evidence chain when a protocol or venue requires it, it comes from a declared primary or final evaluation, contradicts the headline, affects safety, fairness, leakage, or generalization, or materially changes the claim. Otherwise, do not proactively catalogue it merely for completeness; omit it from the main narrative, summarize it collectively, or move it to supplementary material when it adds useful context.
- Narrative reconstruction may change order, emphasis, section boundaries, and the headline within the verified record. It MAY elevate an outcome-informed finding or metric as a post hoc exploratory center when substantively justified; state the selection context and keep the declared primary or final result visible whenever it is needed to interpret that finding. Do not retrospectively claim prior intent or make the headline materially misleading.
...
14. Required prespecified outcomes, declared final evaluations, headline-changing unfavorable findings, uncertainty needed to interpret reported effects, and material safety, fairness, leakage, or applicability boundaries remain correctly labeled; omitted exploratory detail does not change the meaning of the manuscript's claims.
